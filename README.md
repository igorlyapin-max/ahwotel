...
Settings → OTLP export, endpoint вида `https://telemetry.example.org/v1/metrics`. **MVP без аутентификации** по согласованному объёму: только доверенный тестовый ingest; mTLS/JWT/PKI и lifecycle credentials не реализованы. Server TLS/hostname validation обязательны; HTTP, URL credentials/query и redirects запрещены.

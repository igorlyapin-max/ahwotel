...
## Автовозобновление и восстановление стенда — code15

Settings → Sampling: Resume after device restart / Resume when opening the app. Оба переключателя по умолчанию выключены. Сам выбор Continuous или включение переключателя сбор не запускает. Нужен первый явный Start; Stop из приложения/уведомления/SOTI запрещает восстановление. Переключение на timed и административное отключение также снимают разрешение. При включённой хотя бы одной опции Android может восстановить уничтоженный процесс сервиса. WorkManager для выгрузки данных не запускает сбор. Старые установки мигрируют на Room 5 с запрещённым восстановлением; история и очередь сохраняются. Отдельного legacy-механизма автозапуска нет.

Diagnostics показывает разрешение восстановления и последний результат. После reboot сначала разблокируйте устройство. Ограничения фонового запуска OEM/Android могут потребовать открыть приложение вручную. Измерений во время выключенного телефона нет; каждая восстановленная сессия имеет отдельный ID.

На стенде `PROMETHEUS_MEMORY_LIMIT=2g` заменяет фиксированные 768 MiB. Collector удаляет `segment` из labels Prometheus вместе с границами окон: эти значения остаются в локальной истории APK, где используются для разрывов графиков. Это предотвращает рост числа series при каждом изменении контекста; timestamps, source, measurement scope и session ID сохраняются. Старые series удаляются только штатным retention, volumes не очищаются.

Установка на Samsung, проверки и статус суточного наблюдения: [VALIDATION-recovery-code15.md](docs/VALIDATION-recovery-code15.md).

## Защита восстановления при ошибках хранения — code16

DataStore хранит предохранитель `resume_blocked` отдельно от Room. Отсутствующий ключ означает запрет: после обновления нужен новый явный Start. Настройки режима и запрет записываются атомарно до Room-транзакции; незавершённое изменение не может восстановить старую continuous-сессию. Успешное завершение операции разрешает восстановление только для непрерывного сбора. Явный continuous-запуск SOTI остаётся допустимым при timed-настройке по умолчанию.

Stop и административное отключение прекращают runtime независимо от сохранения в Room. Если недоступен также DataStore, запрет гарантируется только в текущем процессе; приложение показывает EN/RU предупреждение о неподтверждённом сохранении. Ошибка записи не выдаётся за подтверждённый запрет после перезагрузки.

Стенд code16 и суточный наблюдатель проверяют container ID, StartedAt, image ID, restart count и `child_generation` supervisor. Ошибочные наблюдения сохраняют конкретные коды и доступные состояния/очередь. `runtime_status()` возвращает structured result с `errors`; ненулевой exit code устанавливает CLI. Новый прогон записывается в отдельный каталог и не смешивается с прежними наблюдениями.

Проверки и незавершённые этапы приёмки: [VALIDATION-recovery-code16.md](docs/VALIDATION-recovery-code16.md).

При проверке code16 обнаружен cgroup OOM Grafana с прежним лимитом 384 MiB. Стенд использует настраиваемый `GRAFANA_MEMORY_LIMIT=1g`; неуспешный прогон сохраняется отдельно, после применения нового лимита суточная проверка начинается с нуля.

Git-поставка `00.00.00.04` / APK code17: [состав, проверки и границы поставки](docs/VALIDATION-00.00.00.04.md). Физические проверки и текущий суточный прогон относятся к установленному code16.
